...
export default function GithubPreviewIssues() {
  ...
  const previewQuery = useGithubPreviewQuery("issues", {
    page: currentPage,
  ...
  const previewErrorMessage = previewQuery.isError
    ? getQueryErrorMessage(previewQuery.error, "Failed to load GitHub issues.")
    : null;
  ...
  async function handleExport() {
    try {
      ...
      downloadPreviewExportFile({
        exportPayload,
        fileNamePrefix: "github-issues-preview",
      });

      toast.success(undefined, {
        description: "GitHub preview exported successfully.",
      });
    } catch (error) {
      ...
      <section className="shrink-0 space-y-3">
        <div className="grid grid-cols-2 gap-2 overflow-visible pb-1 md:items-end md:grid-cols-[minmax(0,1fr)_11.5rem_11.5rem] xl:grid-cols-[minmax(0,1fr)_11.5rem_11.5rem_max-content_auto_auto]">
          ...
          <div className="col-span-2 min-w-0 md:col-span-1">
            <FormSelect
              id="github-preview-issues-repository"
              label="Repository"
          ...
          <div className="min-w-0 md:min-w-[11.5rem]">
            <FormDateSelector
              id="github-preview-issues-start-date"
              label="Start"
              value={startDate}
          ...
          <div className="min-w-0 md:min-w-[11.5rem]">
            <FormDateSelector
              id="github-preview-issues-end-date"
              label="End"
              value={endDate}
          ...
          <div className="col-span-2 grid grid-cols-2 gap-2 md:col-span-3 md:grid-cols-[minmax(0,1fr)_auto_auto] xl:contents">
            ...
            <div className="col-span-2 min-w-0 md:col-span-1">
              <SearchBar
                id="github-preview-issues-search"
                    ...
                    <TableBody>
                      {visibleColumns.length === 0 ? (
                        ...
                        rows.map((row, rowIndex) => (
                          <TableRow key={`preview-issue-row-${rowIndex}`}>
                            {visibleColumns.map((column) => {
                              ...
                              return (
                                ...
                                >
                                  {isObjectLike ? (
                                    ...
                                    <button
                                      type="button"
                                      className="max-w-full cursor-pointer text-left whitespace-normal break-words text-(--color-secondary) hover:underline"
                                      title={rawText}
                                      onClick={() => openCellPreview(value)}
                                    >
                                      {compactText}
