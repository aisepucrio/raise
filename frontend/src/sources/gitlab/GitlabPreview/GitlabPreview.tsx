...
import {
  ...
  useGitlabExportMutation,
  useGitlabOverviewQuery,
  useGitlabPreviewQuery,
  type GitlabExportBody,
  type GitlabPreviewParams,
  type GitlabSection,
} from "@/data";
...
export type GitlabPreviewDateFilterField = "created_at" | "date";

export type GitlabPreviewProps = {
  idPrefix: string;
  previewSection: GitlabSection;
  itemsLabel: string;
  emptyStateMessage: string;
  loadErrorMessage: string;
  exportTable: GitlabExportBody["table"];
  exportFileNamePrefix: string;
  exportSuccessMessage?: string;
  dateFilterField?: GitlabPreviewDateFilterField;
...
function buildDateFilterParams(
  ...
  startDate: string,
  endDate: string,
): Partial<GitlabPreviewParams> {
  if (dateFilterField === "created_at") {
    return {
      ...(startDate ? { created_at__gte: startDate } : {}),
      ...(endDate ? { created_at__lte: endDate } : {}),
    };
  }
...
export function GitlabPreview({
  ...
  exportSuccessMessage = "GitLab preview exported successfully.",
  dateFilterField,
  ...
  const { data: overviewData, isPending: isRepositoryListPending } =
    useGitlabOverviewQuery();
  const previewExportMutation = useGitlabExportMutation();
  ...
  const buildPreviewParams = useCallback(
    ({
      ...
      dateFilters,
    }: PreviewBuildParamsInput): GitlabPreviewParams => ({
      page,
  ...
  const previewQuery = useGitlabPreviewQuery(previewSection, previewParams);
  ...
  const requestExportPayload = useCallback(
    ...
    [previewExportMutation, exportTable],
  );
